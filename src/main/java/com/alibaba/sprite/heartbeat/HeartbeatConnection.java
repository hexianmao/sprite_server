...
import com.alibaba.sprite.net.BackendConnection;
...
import com.alibaba.sprite.net.packet.HeartbeatPacket;
 ...
 * @author xianmao.hexm
 */
public class HeartbeatConnection extends BackendConnection {
    private static final Logger LOGGER = Logger.getLogger(HeartbeatConnection.class);
    ...
    public HeartbeatConnection(SocketChannel channel) {
        super(channel);
        this.handler = new HeartbeatAuthenticator(this);
    ...
    public void heartbeat() {
        if (isAuthenticated) {
            ...
            hp.packetId = 0;
            hp.command = AbstractPacket.COM_HEARTBEAT;
            hp.id = heartbeat.detectCount.incrementAndGet();
            hp.write(this);
            if (HEARTBEAT.isInfoEnabled()) {
                HEARTBEAT.info(heartbeat.requestMessage(Heartbeat.SEND, String.valueOf(hp.id).getBytes()));
            }
        } else {
            authenticate();
        }
    ...
    public void error(int errCode, Throwable t) {
        ...
        case ErrorCode.ERR_HANDLE_DATA:
            heartbeat.setResult(Heartbeat.ERROR_STATUS, this, false, null);
            break;
        default:
            heartbeat.setResult(Heartbeat.ERROR_STATUS, this, true, null);
        }
    }
